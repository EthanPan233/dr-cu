...
void RouteGrid::getAllViaVio(vector<int>& sameLayerViaVios,
    ...
    // TODO:
    std::ofstream sameLayerViaViosFile (db::setting.outputFile + ".sameLayerViaVios");
    std::ofstream viaTopViaViosFile (db::setting.outputFile + ".viaTopViaVios");
    std::ofstream viaBotWireViosFile (db::setting.outputFile + ".viaBotWireVios");
    std::ofstream viaTopWireViosFile (db::setting.outputFile + ".viaTopWireVios");
    std::ofstream poorViaFile (db::setting.outputFile + ".poorVia");
    ...
    for (unsigned layerIdx = 0; (layerIdx + 1) != getLayerNum(); ++layerIdx) {
        sameLayerViaViosFile << "Layer " << layerIdx << std::endl;
        viaTopViaViosFile << "Layer " << layerIdx << std::endl;
        viaBotWireViosFile << "Layer " << layerIdx << std::endl;
        viaTopWireViosFile << "Layer " << layerIdx << std::endl;
        poorViaFile << "Layer " << layerIdx << std::endl;
        for (unsigned trackIdx = 0; trackIdx != layers[layerIdx].numTracks(); ++trackIdx) {
            for (const std::pair<int, int>& p : routedViaMap[layerIdx][trackIdx]) {
                ...
                int temp = getViaUsageOnSameLayerVias(via, p.second, viaType);
                if (temp != 0) sameLayerViaViosFile << trackIdx << " " << p.first << std::endl;
                sameLayerViaVios[layerIdx] += temp;

                temp = getViaUsageOnTopLayerVias(via, p.second, viaType);
                if (temp != 0) viaTopViaViosFile << trackIdx << " " << p.first << std::endl;
                viaTopViaVios[layerIdx] += temp;

                temp = getViaUsageOnBotWires(via, p.second, viaType);
                if (temp != 0) viaBotWireViosFile << trackIdx << " " << p.first << std::endl;
                viaBotWireVios[layerIdx] += temp;

                temp = getViaUsageOnTopWires(via, p.second, viaType);
                if (temp != 0) viaTopWireViosFile << trackIdx << " " << p.first << std::endl;
                viaTopWireVios[layerIdx] += temp;
                if (getViaPoorness(via, p.second) == ViaPoorness::Poor) {
                    poorViaFile << trackIdx << " " << p.first << std::endl;
                    ++poorVia[layerIdx];
                }
            }
        }
    ...
    sameLayerViaViosFile.close();
    viaTopViaViosFile.close();
    viaBotWireViosFile.close();
    viaTopWireViosFile.close();
    poorViaFile.close();
...
void RouteGrid::getAllWireShortVio(vector<int>& shortNum, vector<DBU>& shortLen) const {
    // TODO: 
    std::ofstream myfile (db::setting.outputFile + ".wireShortVio");
    shortNum.assign(getLayerNum(), 0);
    shortLen.assign(getLayerNum(), 0.0);
    for (int layerIdx = 0; layerIdx < getLayerNum(); ++layerIdx) {
        myfile << "Layer " << layerIdx << std::endl;
        for (int trackIdx = 0; trackIdx < routedWireMap[layerIdx].size(); trackIdx++) {
            auto& track = routedWireMap[layerIdx][trackIdx];
        // for (const auto& track : routedWireMap[layerIdx]) {
            for (const auto& intvlUsage : track) {
                const auto& intvl = intvlUsage.first;
                int usage = intvlUsage.second.size();
                if (usage > 1) {
                    ...
                    if (usage - 1 >= 1) {
                        for (int i = first(intvlUsage.first); i <= last(intvlUsage.first); i++) {
                            myfile << trackIdx << " " << i << std::endl;
                        }
                    }
                    shortLen[layerIdx] +=
...
void RouteGrid::getAllPoorWire(vector<int>& num, vector<DBU>& len) const {
    // TODO: 
    std::ofstream myfile (db::setting.outputFile + ".poorWire");
                ...
                iteratePoorWireSegments({layerIdx, trackIdx, {first(intvl), last(intvl)}},
                                        *intvlUsage.second.begin(),
                                        [&](const utils::IntervalT<int>& poorIntvl) {
                                            ++num[layerIdx];
                                            for (int i = poorIntvl.low; i <= poorIntvl.high; i++) {
                                                myfile << trackIdx << " " << i << std::endl;
                                            }
...
void RouteGrid::getAllWireSpaceVio(vector<int>& spaceVioNum) const {
    std::ofstream myfile (db::setting.outputFile + ".wireSpaceVio");
